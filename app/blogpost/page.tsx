...
const BlogPost: React.FC<{ blogTitle: string; count: number }> = ({ blogTitle, count }) => {
    return <Link prefetch={true} className="w-full h-full flex justify-center items-center bg-[#f5f5f5] p-[1rem]" href={`/blogpost/blogpost-${count}-${blogTitle}`}>{`blogpost ${count}`}</Link>;
};
